...
$.fn.callLightboxSelected=function(){$('.lightbox_selected').lightGallery({selector:'a.lightbox_item',download:true,speed:500,loop:true,controls:true,thumbWidth:100,thumbContHeight:100,thumbMargin:8,thumbnail:true,share:true,cssEasing:'cubic-bezier(0.645, 0.045, 0.355, 1)',loadYoutubeThumbnail:true,youtubeThumbSize:'default',loadVimeoThumbnail:true,iframeMaxWidth:'75%',vimeoThumbSize:'thumbnail_medium',youtubePlayerParams:{modestbranding:1,showinfo:0,rel:0,controls:0},vimeoPlayerParams:{byline:0,portrait:0,color:'A90707'}});}
if($(".lightbox_selected").exists()){$(window).callLightboxSelected();}
$.fn.callLightbox=function(){$('.lightbox').lightGallery({selector:'this',download:true,thumbWidth:100,thumbContHeight:100,thumbnail:true,share:true,loadYoutubeThumbnail:true,youtubeThumbSize:'default',iframeMaxWidth:'75%',loadVimeoThumbnail:true,youtubePlayerParams:{modestbranding:1,showinfo:0,rel:0,controls:0},vimeoPlayerParams:{byline:0,portrait:0,color:'A90707'}});}
...
'<div><span class="time">'+seconds+'</span>'+'<span class="datename"> seconds </span></div></div>';$($this).html(datestr);if(years<1){$($this).find(".year, .year + .dot").hide();}}showRemaining(),timer=setInterval(showRemaining,1000);}});};/*!
...
if(typeof imgs[i].node.edge_media_to_caption.edges[0]!=="undefined"){caption=imgs[i].node.edge_media_to_caption.edges[0].node.text;}else if(typeof imgs[i].node.accessibility_caption!=="undefined"){caption=imgs[i].node.accessibility_caption;}else{caption=(is_tag?data.name:data.username)+" image "+i;}
...
html+="</div>";}}
$(options.container).html(html);}).fail(function(e){console.error("Instagram Feed: Unable to fetch the given user/tag. Instagram responded with the status code: ",e.status);})
return true;};})(jQuery);